...
private:
	mutable ofstream output_stream;
	TreeNode*		 root_node;
};
...
int get_op_code(const string& op) {
	static const vector<string> command = { "+", "-", "depth", "leaf" };
...
void BST::insert_node(const string & data) {
	if (root_node == nullptr)
		...
		insert_node = [&data, &insert_node](TreeNode* node) {
			if (node->value > data) {
				if (node->left)
...
TreeNode * TreeNode::get_min_node(void) {
